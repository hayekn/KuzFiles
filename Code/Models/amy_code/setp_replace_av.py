...
y0 = [0.5, 0, 0.1, 0.2, 0, 0] #seek, setp, binge,  nac, av, ALCOHOL, vta
...
binTOnac = 1
vtaTOnac = 1
avTOseek = 0 #remove connection from av to seek
csTOseek = 10 #may want to change to zero
csTOvta = 3
seekExcite = 4.2
bingeExcite = 0
avTOsetp = 0.15
...
def binge_model(t, y0):
    def model(t, y):
        seek, setp, binge, nac,  ALCOHOL, vta = y
        
        CS = np.heaviside(csDUR-t,0.5) #Conditioned Stimulus
        dseek_dt = (-seek + F(Eseek * (seekExcite * seek + csTOseek * CS - spTOseek * setp - seekDRIVE))) / seekTAU #Seek Activity
        # dsetp_dt = (-setp + np.exp(-decayFac*t) * F(Esetp * (avTOsetp*av - setpDRIVE))) / setpTAU  #Setpoint Activity       
        dbinge_dt = (-binge + F(Ebinge * (bingeExcite * binge + seekTObin * seek - bingeDRIVE))) / bingeTAU #Binge Activity
        dnac_dt = (-nac + F(Enac * (vtaTOnac * vta + seekTOnac * seek + binTOnac * binge - nacDRIVE))) / nacTAU #NAc Activity
        dsetp_dt = (-setp + F(Eav * nac - avDRIVE)) / avTAU #Alcohol Variable
        dALCOHOL_dt = nac # Alcohol consumed 
        dvta_dt = (-vta + F(Evta*(csTOvta * CS - vtaDRIVE))) / vtaTAU #VTA activity
        
        return [dseek_dt, dsetp_dt, dbinge_dt, dnac_dt,  dALCOHOL_dt, dvta_dt]
...
def der_model(t, y): #This model is also defined separately, just for convenience. The system of ODEs is also defined in the "binge_model"
        ...
        CS = np.heaviside(csDUR-t,0.5) #Conditioned Stimulus
        dseek_dt = (-seek + F(Eseek * (seekExcite * seek + csTOseek * CS - spTOseek * setp + avTOseek * av - seekDRIVE))) / seekTAU #Seek Activity
        dsetp_dt = (-setp + np.exp(-decayFac*t) * F(Esetp * (avTOsetp*av - setpDRIVE))) / setpTAU  #Setpoint Activity       
        dbinge_dt = (-binge + F(Ebinge * (bingeExcite * binge + seekTObin * seek - bingeDRIVE))) / bingeTAU #Binge Activity
        dnac_dt = (-nac + F(Enac * (vtaTOnac * vta + seekTOnac * seek + binTOnac * binge - nacDRIVE))) / nacTAU #NAc Activity
