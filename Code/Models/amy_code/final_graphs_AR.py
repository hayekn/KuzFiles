...
y0 = [0.1, 0, 0.1, 0.1, 0, 0, 0, EnacMEAN, driveMEAN] #setp, seek, binge, nac, dls, vta, ALCOHOL, Enac, nacDRIVE
y_traj = [0.1, 0, 0.1, 0.1, 0, 0, 0, EnacMEAN, driveMEAN]
...
############ Graphs ################
def subplots(y, time=120, noise=False, save_sub=False, save_ani = False, anim=False):
    ...
    frames=300
    def update(frame):
        y = xppaut_model(t, y0, noise, csTOvta=3.5*(frame/frames)+0) # seekTObin=1*(frame/frames)+2.6, binTOseek=1*(frame/frames)+2.6
...
def ind_plots(y, graph, csTOvta=csTOvta, t=t, save=False):
    #graph: PFC, Insula, STR, VTA, Alc, Param, Cortex, Subcort
    ...
    if graph == 'PFC' or 'Insula' or 'STR' or 'VTA' or 'Cortex' or 'Subcort':
        ...
        if save == True:
            f.savefig('/Users/amyrude/Downloads/graph_'+graph+'_updated.png',transparent=False, dpi = 350)   
...
def DA_graphs(F, S, M, L, save = False):
    ...
    f1, ax = plt.subplots(1,2, figsize = (7,5))
    ...
    if save == True:
        f1.savefig('/Users/amyrude/Downloads/VTA_NAc.png',transparent=True, dpi = 350)
    ...
    plt.tight_layout()
    if save == True:
        f2.savefig('/Users/amyrude/Downloads/DA_Alc.png',transparent=True, dpi = 350)

    f3, ax = plt.subplots(3,1, figsize = (7,7))
    for k in range(3):
        ax[k].clear()
        ax[k].set_xlim(0,60)
    ...
    if save == True:
        f2.savefig('/Users/amyrude/Downloads/DA_comb.png',transparent=True, dpi = 350)

    param_array = np.linspace(0, 3.5, 200)
    final_alcohol = []
    peak_vta = []
    peak_nac = []

    for n in np.arange(len(param_array)):
         y= xppaut_model(t, y0, csTOvta = param_array[n])['Int']
         alc = y[6]
         final_alcohol.append(alc[-1])
         peak_vta.append(np.max(y[5]))
         peak_nac.append(np.max(y[3]))

    f4, ax1 = plt.subplots(figsize = (8,6))
    color = 'tab:red'
    ...
    if save == True:
        f4.savefig('/Users/amyrude/Downloads/DA_VTA_peak.png',transparent=True, dpi = 350)
